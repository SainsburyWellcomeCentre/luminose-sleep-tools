import h5py
import numpy as np
import matplotlib.pyplot as plt
from sleep_tools.io import SleepRecording
from sleep_tools.analysis import SleepAnalyzer

mat_path = 'example_data/comparison_with_spike2_to_debug_inconsistencies/sub-003_ses-001_recording-001_0001_2026-03-25_16_13_47_0to300s-using_spike2.mat'
edf_path = 'example_data/comparison_with_spike2_to_debug_inconsistencies/large_file_sub-003_ses-001_recording-001_0001_2026-03-25_16_13_47.edf'

# Load MAT
with h5py.File(mat_path, 'r') as mat:
    mat_eeg_raw = mat['V3_ses_001_recording_001_0001_2026_03_25_16_13_47_EEG_EEG1A_B/values'][0]
    mat_eeg_filt = mat['b_003_ses_001_recording_001_0001_2026_03_25_16_13_47_EEGFilt/values'][0]
    mat_emg_raw = mat['b_003_ses_001_recording_001_0001_2026_03_25_16_13_47_EMG_EMG/values'][0]
    mat_emg_filt = mat['b_003_ses_001_recording_001_0001_2026_03_25_16_13_47_EMGFilt/values'][0]
    mat_delta = mat['V3_ses_001_recording_001_0001_2026_03_25_16_13_47_Delta_4001_/values'][0]
    mat_theta = mat['V3_ses_001_recording_001_0001_2026_03_25_16_13_47_Theta_4001_/values'][0]
    mat_td = mat['V03_ses_001_recording_001_0001_2026_03_25_16_13_47_T_D__4001_/values'][0]

    # Time axes
    mat_eeg_times = mat['V3_ses_001_recording_001_0001_2026_03_25_16_13_47_EEG_EEG1A_B/times'][0]
    mat_pow_times = mat['V3_ses_001_recording_001_0001_2026_03_25_16_13_47_Delta_4001_/times'][0]

print(f"MAT loaded. EEG points: {len(mat_eeg_raw)}, Power points: {len(mat_delta)}")

# Load EDF and run sleep-tools
rec = SleepRecording.load(edf_path)
analyzer = SleepAnalyzer(rec)

# The MAT file is 0 to 300s. The EDF might be longer.
# We'll just compute features for the full file or up to 300s
# Actually compute_all_features processes the whole recording, but if it's large it might take a moment.
print("Computing sleep-tools features...")
features = analyzer.compute_all_features()

# Interpolate sleep-tools features onto MAT time points to compare
# EEG filtering
st_eeg_times = np.arange(len(features['eeg_filtered'])) / rec.sfreq
st_eeg_filt = features['eeg_filtered']
st_eeg_filt_interp = np.interp(mat_eeg_times, st_eeg_times, st_eeg_filt)

st_emg_filt = features['emg_filtered']
st_emg_filt_interp = np.interp(mat_eeg_times, st_eeg_times, st_emg_filt)

st_delta = features['delta_power']
st_theta = features['theta_power']
st_td = features['td_ratio']
st_pow_times = features['times']

st_delta_interp = np.interp(mat_pow_times, st_pow_times, st_delta)
st_theta_interp = np.interp(mat_pow_times, st_pow_times, st_theta)
st_td_interp = np.interp(mat_pow_times, st_pow_times, st_td)

# Print some comparisons
print("\n=== Comparisons ===")
print(f"EEG Filt MAE: {np.mean(np.abs(mat_eeg_filt - st_eeg_filt_interp)):.5f}")
print(f"EMG Filt MAE: {np.mean(np.abs(mat_emg_filt - st_emg_filt_interp)):.5f}")

# Remember Spike2 might scale things differently. Spike2's channel definitions have "Scale" and "Divide".
# In OSD.s2cx, EMG is in mV! "Units=mV", "Comment=x1000", "Scale=100", "Divide=27". Wait.
# Let's just calculate correlation coefficients
print(f"EEG Filt Corr: {np.corrcoef(mat_eeg_filt, st_eeg_filt_interp)[0,1]:.5f}")
print(f"EMG Filt Corr: {np.corrcoef(mat_emg_filt, st_emg_filt_interp)[0,1]:.5f}")
print(f"Delta Corr: {np.corrcoef(mat_delta, st_delta_interp)[0,1]:.5f}")
print(f"Theta Corr: {np.corrcoef(mat_theta, st_theta_interp)[0,1]:.5f}")
print(f"T:D Corr: {np.corrcoef(mat_td, st_td_interp)[0,1]:.5f}")

# Check T:D ratio manually
mat_manual_td = mat_theta / (mat_delta + 1e-12)
print(f"MAT T:D vs MAT Manual T:D Corr: {np.corrcoef(mat_td, mat_manual_td)[0,1]:.5f}")
print(f"MAT T:D Mean: {np.mean(mat_td):.5f}, MAT Manual T:D Mean: {np.mean(mat_manual_td):.5f}")
